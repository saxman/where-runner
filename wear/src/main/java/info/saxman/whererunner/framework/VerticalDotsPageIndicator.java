package info.saxman.whererunner.framework;
...
public class VerticalDotsPageIndicator extends LinearLayout {
    ...
    public VerticalDotsPageIndicator(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, info.saxman.whererunner.R.styleable.VerticalDotsPageIndicator, 0, 0);

        try {
            mDotSpacing = a.getDimensionPixelOffset(info.saxman.whererunner.R.styleable.VerticalDotsPageIndicator_dotSpacing, 0);
            mDotDrawableId = a.getResourceId(info.saxman.whererunner.R.styleable.VerticalDotsPageIndicator_dotDrawable, 0);
            mDotDrawableCurrentId = a.getResourceId(info.saxman.whererunner.R.styleable.VerticalDotsPageIndicator_dotDrawableCurrent, 0);
        } finally {
            a.recycle();
        }
    }
    ...
    public void setPager(GridViewPager pager) {
        ...
        if (mPipImageViews.length > 0) {
            mPipImageViews[0].setImageResource(info.saxman.whererunner.R.drawable.ic_page_indicator_dot_current);
        }
